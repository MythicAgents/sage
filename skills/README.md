...
| **sage-focused-capability-tests** | Narrow live/semi-live smoke tests for one capability/adapter (ADCS / DCSync / GPO / LAPS / Kerberos), plus the fixed-content Slack findings-delivery probe. | `scripts/build_capability_smoke.py`, `scripts/run_focused_*.py`, `scripts/probe_slack_findings_webhook.py` |
