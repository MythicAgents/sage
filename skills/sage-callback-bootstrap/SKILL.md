...
`bootstrap-reset` must report a running `sage_chat_container`, a wildcard-scoped API token owned by the current
operation's unique active bot, and an empty locked prepared channel named
