    ...
    *,
    ...
    return ";".join([
        ...
        (
            ...
            # DA-only read + negative control: reading the domain object's SACL requires
            # SeSecurityPrivilege (admins only). Run the identical read on the cert-authenticated
            # DA session AND on a Negotiate-bound foothold session. DA_ONLY_READ_PROVEN is True only
            # when the DA session retrieves the SACL and an authenticated non-admin is denied it —
            # a differential a self-read cannot fake (RCA 2026-08-01).
            "$lines.Add('DA_ONLY_READ_OP=domain-sacl');"
            "$saclProbe={param($connection,$baseDn) try{"
            "$req=New-Object System.DirectoryServices.Protocols.SearchRequest($baseDn,'(objectClass=*)',[System.DirectoryServices.Protocols.SearchScope]::Base,[string[]]@('nTSecurityDescriptor'));"
            "$sdControl=New-Object System.DirectoryServices.Protocols.SecurityDescriptorFlagControl([System.DirectoryServices.Protocols.SecurityMasks]::Sacl);"
            "$null=$req.Controls.Add($sdControl);"
            "$resp=$connection.SendRequest($req);"
            "if($resp.Entries.Count -lt 1){return (New-Object psobject -Property @{Status='empty';Aces=-1})};"
            "$e=$resp.Entries[0];"
            "if(-not $e.Attributes['nTSecurityDescriptor']){return (New-Object psobject -Property @{Status='no-sd';Aces=-1})};"
            "$raw=$e.Attributes['nTSecurityDescriptor'][0];"
            "if($raw -isnot [byte[]]){return (New-Object psobject -Property @{Status='no-bytes';Aces=-1})};"
            "$sd=New-Object System.Security.AccessControl.RawSecurityDescriptor($raw,0);"
            "if($sd.SystemAcl -eq $null){return (New-Object psobject -Property @{Status='no-sacl';Aces=-1})};"
            "return (New-Object psobject -Property @{Status='sacl';Aces=[int]$sd.SystemAcl.Count})"
            "}catch{$m=$_.Exception.Message -replace \"`r|`n\",' ';return (New-Object psobject -Property @{Status=('error:'+$m);Aces=-1})}};"
            "$daSacl=(& $saclProbe $conn $base);"
            "$lines.Add(('DA_SACL_STATUS='+$daSacl.Status));"
            "$lines.Add(('DA_SACL_ACE_COUNT='+[string]$daSacl.Aces));"
            "$controlConn=$null;$controlSacl=$null;"
            "try{"
            "$cid=New-Object System.DirectoryServices.Protocols.LdapDirectoryIdentifier($server,636,$true,$false);"
            "$controlConn=New-Object System.DirectoryServices.Protocols.LdapConnection($cid);"
            "$controlConn.AuthType=[System.DirectoryServices.Protocols.AuthType]::Negotiate;"
            "$controlConn.SessionOptions.SecureSocketLayer=$true;"
            "$controlConn.SessionOptions.ProtocolVersion=3;"
            "$controlConn.SessionOptions.ReferralChasing=[System.DirectoryServices.Protocols.ReferralChasingOptions]::None;"
            "$controlConn.SessionOptions.VerifyServerCertificate={param($c,$crt) $true};"
            "$controlConn.Bind();"
            "$lines.Add('CONTROL_BIND=True');"
            "$cwho='';try{$cw=$controlConn.SendRequest((New-Object System.DirectoryServices.Protocols.ExtendedRequest('1.3.6.1.4.1.4203.1.11.3')));if($cw.ResponseValue){$cwho=[Text.Encoding]::UTF8.GetString($cw.ResponseValue)}}catch{};"
            "$lines.Add(('CONTROL_WHOAMI='+$cwho));"
            "$controlSacl=(& $saclProbe $controlConn $base);"
            "$lines.Add(('CONTROL_SACL_STATUS='+$controlSacl.Status));"
            "$lines.Add(('CONTROL_SACL_ACE_COUNT='+[string]$controlSacl.Aces))"
            "}catch{$m=$_.Exception.Message -replace \"`r|`n\",' ';$lines.Add('CONTROL_BIND=False');$lines.Add(('CONTROL_BIND_ERROR='+$m))};"
            "$daGot=($daSacl.Status -eq 'sacl');"
            "$controlDenied=(($controlConn -ne $null) -and ($controlSacl -ne $null) -and ($controlSacl.Status -ne 'sacl'));"
            "$lines.Add(('DA_ONLY_READ_PROVEN='+[string]([bool]($daGot -and $controlDenied))));"
            "$res=$searchResponse;"
