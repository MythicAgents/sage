...
2. INGEST → the collection comes back as a Mythic file artifact (a file UUID from the `download` task). Mythic_Operator calls `ingest_collection(callback_display_id=N)` (or `file_uuid=...`), which fetches the bytes and uploads them straight into BloodHound IN-MEMORY (no disk staging). Then the BloodHound agent runs `domain_info`/`data_quality` to confirm the ACL/GPO/Trust edges are present (ingest is async — allow a few seconds).
